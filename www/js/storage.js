...
window.Storage = {
  ...
  async saveConfig() {
    try {
      const saveEnabled = document.getElementById("saveCredentials").checked;
      ...
      const config = {
        ...
        registerOnStartup: document.getElementById("registerOnStartup").checked,
        hideEventLog: document.getElementById("hideEventLog").checked,
        enableCallRecording: document.getElementById("enableCallRecording")
          .checked,
  ...
  async loadConfig() {
    try {
      ...
      const registerOnStartupEl = document.getElementById("registerOnStartup");
      const hideEventLogEl = document.getElementById("hideEventLog");
      const enableCallRecordingEl = document.getElementById(
        "enableCallRecording",
      );
      const saveCredentialsEl = document.getElementById("saveCredentials");

      if (hideCallerIDEl) hideCallerIDEl.checked = config.hideCallerID || false;
      if (registerOnStartupEl)
        registerOnStartupEl.checked = config.registerOnStartup || false;
      if (hideEventLogEl) hideEventLogEl.checked = config.hideEventLog || false;
      if (enableCallRecordingEl)
        enableCallRecordingEl.checked = config.enableCallRecording || false;
      ...
      if (config.registerOnStartup && config.username && config.password) {
        setTimeout(() => {
          window.log("Auto-registering...");
          if (typeof window.register === "function") {
            window.log("Calling register function for auto-registration");
            window.register();
          } else {
  ...
  async addRecordingToHistory(recording) {
    try {
      const recordings = (await localforage.getItem(this.RECORDINGS_KEY)) || [];
