...
window.startRecording = async function () {
  ...
  try {
    ...
    window.log(`🎙️ Starting recording - Platform: ${isIOS ? "iOS" : "Other"}`);

    // On iOS, wait longer for iosrtc audio tracks to be ready
    if (isIOS) {
      window.log("⏳ [iOS] Waiting 1.5 seconds for iosrtc audio tracks...");
    ...
    // METHOD 1: Standard WebRTC API (getReceivers/getSenders)
    window.log("📡 Method 1: Checking standard WebRTC API...");
    const receivers = pc.getReceivers();
    window.log(`  Found ${receivers.length} receivers`);
    receivers.forEach((receiver, idx) => {
      if (receiver.track && receiver.track.kind === "audio") {
        window.log(
          `  Receiver ${idx}: id=${receiver.track.id}, enabled=${receiver.track.enabled}, muted=${receiver.track.muted}, readyState=${receiver.track.readyState}`,
        );
        if (receiver.track.readyState === "live") {
          try {
            ...
            tracksAdded++;
            window.log(`  ✅ Added receiver track ${idx}`);
          } catch (e) {
            window.log(
              `  ❌ Failed to add receiver track ${idx}: ${e.message}`,
            );
          }
        } else {
          window.log(`  ⊗ Skipped receiver ${idx} (not live)`);
        }
      }
    });
    ...
    senders.forEach((sender, idx) => {
      if (sender.track && sender.track.kind === "audio") {
        window.log(
          `  Sender ${idx}: id=${sender.track.id}, enabled=${sender.track.enabled}, muted=${sender.track.muted}, readyState=${sender.track.readyState}`,
        );
        if (sender.track.readyState === "live") {
          try {
            ...
            tracksAdded++;
            window.log(`  ✅ Added sender track ${idx}`);
          } catch (e) {
            window.log(`  ❌ Failed to add sender track ${idx}: ${e.message}`);
          }
        } else {
          window.log(`  ⊗ Skipped sender ${idx} (not live)`);
        }
      }
    });

    // METHOD 2: Legacy WebRTC API (for iosrtc compatibility)
    if (tracksAdded === 0 && isIOS) {
      window.log("📡 Method 2: Trying legacy WebRTC API (iosrtc)...");

      if (typeof pc.getRemoteStreams === "function") {
        const remoteStreams = pc.getRemoteStreams();
        window.log(`  Found ${remoteStreams.length} remote streams`);
        remoteStreams.forEach((stream, streamIdx) => {
          const audioTracks = stream.getAudioTracks();
          window.log(
            `  Remote stream ${streamIdx} has ${audioTracks.length} audio tracks`,
          );
          audioTracks.forEach((track, trackIdx) => {
            window.log(
              `    Track ${trackIdx}: id=${track.id}, state=${track.readyState}, enabled=${track.enabled}`,
            );
            if (track.readyState === "live") {
              try {
                ...
                tracksAdded++;
                window.log(
                  `    ✅ Added remote stream track ${streamIdx}:${trackIdx}`,
                );
              } catch (e) {
                window.log(`    ❌ Failed to add track: ${e.message}`);
              }
            }
          });
        });
      ...
      if (typeof pc.getLocalStreams === "function") {
        ...
        window.log(`  Found ${localStreams.length} local streams`);
        localStreams.forEach((stream, streamIdx) => {
          const audioTracks = stream.getAudioTracks();
          window.log(
            `  Local stream ${streamIdx} has ${audioTracks.length} audio tracks`,
          );
          audioTracks.forEach((track, trackIdx) => {
            window.log(
              `    Track ${trackIdx}: id=${track.id}, state=${track.readyState}, enabled=${track.enabled}`,
            ...
            if (track.readyState === "live") {
              try {
                ...
                tracksAdded++;
                window.log(
                  `    ✅ Added local stream track ${streamIdx}:${trackIdx}`,
                );
              } catch (e) {
                window.log(`    ❌ Failed to add track: ${e.message}`);
              }
    ...
    // METHOD 3: Audio elements (last resort for iOS)
    if (tracksAdded === 0 && isIOS) {
      window.log("📡 Method 3: Trying audio elements...");

      const remoteAudio = document.getElementById("remoteAudio");
      if (remoteAudio && remoteAudio.srcObject) {
        const stream = remoteAudio.srcObject;
        const audioTracks = stream.getAudioTracks();
        window.log(`  remoteAudio has ${audioTracks.length} tracks`);
        audioTracks.forEach((track, idx) => {
          window.log(
            `    Track ${idx}: id=${track.id}, state=${track.readyState}`,
          );
          if (track.readyState === "live") {
            try {
              const source = recordingAudioContext.createMediaStreamSource(
              ...
              source.connect(window.recordingDestination);
              tracksAdded++;
              window.log(`    ✅ Added remoteAudio track ${idx}`);
            } catch (e) {
              window.log(`    ❌ Failed: ${e.message}`);
            }
          }
        });
      } else {
        window.log("  ⊗ remoteAudio not available");
      }
      ...
      if (localAudio && localAudio.srcObject) {
        ...
        const audioTracks = stream.getAudioTracks();
        window.log(`  localAudio has ${audioTracks.length} tracks`);
        audioTracks.forEach((track, idx) => {
          window.log(
            `    Track ${idx}: id=${track.id}, state=${track.readyState}`,
          );
          if (track.readyState === "live") {
            try {
              ...
              source.connect(window.recordingDestination);
              tracksAdded++;
              window.log(`    ✅ Added localAudio track ${idx}`);
            } catch (e) {
              window.log(`    ❌ Failed: ${e.message}`);
            }
          }
        ...
        window.log("  ⊗ localAudio not available");
      }
    }

    if (tracksAdded === 0) {
      throw new Error(
        "No audio tracks available for recording after trying all methods",
      );
    }

    window.log(`✅ Connected ${tracksAdded} audio tracks to recording stream`);
    window.log(
      `📊 Recording stream has ${recordingStream.getAudioTracks().length} output tracks`,
    );
    ...
    if (!MediaRecorder.isTypeSupported) {
      throw new Error("MediaRecorder.isTypeSupported not available");
    }
    ...
    if (!mimeType) {
      ...
      window.log(`No preferred format supported, using default: ${mimeType}`);
    } else {
    ...
    mediaRecorder.onerror = (event) => {
      window.log(`Recording error: ${event.error || "Unknown error"}`);
    ...
    if (isNative) {
      ...
      if (CallService?.saveRecording) {
        try {
          ...
          await window.Storage.updateCallHistoryWithRecording(filename);
          await window.Storage.addRecordingToHistory({
            filename: filename,
            callerNumber: callerNumber,
            direction: recordingCallDirection || "unknown",
            timestamp: callTimestamp,
            duration: getCallDuration(),
          });
          return;
        } catch (error) {
          console.error("Native plugin save failed:", error);
        }
      }
